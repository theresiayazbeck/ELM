...
#include "shr_assert.h"
  ...
  use landunit_varcon   , only : istsoil, istcrop, istdlak, istwet
    ...
    do ib = 1, nband
       do fc = 1,num_nourbanc
          c = filter_nourbanc(fc)
             ...
             do i = -nlevsno+1,1,1
              if (subgridflag == 0 .or. lun_pp%itype(col_pp%landunit(c)) == istdlak .or. lun_pp%itype(col_pp%landunit(c)) == istwet) then
                if (ib == 1) then
       ...
       p = filter_nourbanp(fp)
          if (coszen_patch(p) > 0._r8) then
             if ((lun_pp%itype(veg_pp%landunit(p)) == istsoil .or.  &
                  lun_pp%itype(veg_pp%landunit(p)) == istcrop .or.  &
                  lun_pp%itype(veg_pp%landunit(p)) == istwet ) &
