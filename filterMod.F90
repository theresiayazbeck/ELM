...
#include "shr_assert.h"
  ...
  type clumpfilter
     ...
     integer, pointer :: wetsoilc(:)     ! wetland soil filter (columns)
     integer :: num_wetsoilc             ! number of columns in wetland soil filter 
     integer, pointer :: wetsoilp(:)     ! wetland soil filter (pfts)
     integer :: num_wetsoilp             ! number of pfts in wetland soil filter
...
!$OMP PARALLEL DO PRIVATE (nc,bounds)
    do nc = 1, nclumps
       ...
       allocate(this_filter(nc)%wetsoilc(bounds%endc-bounds%begc+1))
       allocate(this_filter(nc)%wetsoilp(bounds%endp-bounds%begp+1))
...
!$OMP END PARALLEL DO
  ...
  subroutine setFiltersOneGroup(bounds, this_filter, include_inactive, icemask_grc )
    ...
    ! Create wetland soil filter at column-level
       ...
       if (top_pp%active(t)) then
          if (col_pp%active(c) .or. include_inactive) then
             ...
             if (lun_pp%itype(l) == istwet) then
                fs = fs + 1
                this_filter(nc)%wetsoilc(fs) = c
             end if
          end if
       end if
    end do
    this_filter(nc)%num_wetsoilc = fs
    ...
    ! Create wetland soil filter at pft-level
    ...
    do p = bounds%begp,bounds%endp
       ...
       if (top_pp%active(t)) then
          if (veg_pp%active(p) .or. include_inactive) then
             ...
             if (lun_pp%itype(l) == istwet) then
                fs = fs + 1
                this_filter(nc)%wetsoilp(fs) = p
             end if
          end if
       end if
    end do
    this_filter(nc)%num_wetsoilp = fs
