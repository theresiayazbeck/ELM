...
contains
  ...
  subroutine ColNBalanceCheck(bounds, &
    ...
    associate(                                                                             &
         ...
         if(use_fates) then
            ...
            do p = col_pp%pfti(c), col_pp%pftf(c)
               if (veg_pp%active(p) .and. (veg_pp%itype(p) .ne. noveg)) then
                  ...
                  write(iulog,*)'p,supplement_to_plantn(p),veg_pp%wtcol(p),col_ninputs(c),col_ninputs(c)',&
                      p,supplement_to_plantn(p),veg_pp%wtcol(p),col_ninputs(c),col_ninputs(c) 
               end if
            end do
      ...
      write(iulog,*)'col_noutputs components: denit(c),col_fire_nloss(c),col_prod1n_loss(c),col_prod10n_loss(c),col_prod100n_loss(c),',&
           'f_n2o_nit(c),som_n_leached(c),col_noutputs(c)',&
           denit(c),col_fire_nloss(c),col_prod1n_loss(c),col_prod10n_loss(c),col_prod100n_loss(c),f_n2o_nit(c),som_n_leached(c),col_noutputs(c)
      write(iulog,*)'col_ninput components: ndep_to_sminn(c),nfix_to_sminn(c),supplement_to_sminn(c),col_ninputs(c)',&
           ndep_to_sminn(c),nfix_to_sminn(c),supplement_to_sminn(c),col_ninputs(c)
    ...
    associate(                                                                            &
      ...
      do fc = 1,num_soilc
         ...
         if(use_fates) then
            ...
            do p = col_pp%pfti(c), col_pp%pftf(c)
               write(iulog,*)'c,p,veg_pp%active(p),veg_pp%itype(p)',c,p,veg_pp%active(p),veg_pp%itype(p)
               if (veg_pp%active(p) .and. (veg_pp%itype(p) .ne. noveg)) then
