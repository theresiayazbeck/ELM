...
contains
  ...
  subroutine control_init( )
    ...
    namelist /elm_inparm/  &
         clump_pproc, wrtdia, &
         create_crop_landunit, create_wetland_landunit, nsegspc, co2_ppmv, override_nsrest, &
    ...
    if (masterproc) then
       ...
       if (create_wetland_landunit .and. .not. use_lch4) then
          call endrun(msg=' ERROR: create_wetland_landunit requires use_lch4=.true.'//&
               errMsg(__FILE__, __LINE__))
       end if
  ...
  subroutine control_spmd()
    ...
    ! Wetland Landunit generation
    call mpi_bcast(create_wetland_landunit, 1, MPI_LOGICAL, 0, mpicom, ier)
