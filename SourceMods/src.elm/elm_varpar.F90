module elm_varpar
  ...
  use elm_varctl   , only: use_extralakelayers, use_vertsoilc, use_crop, use_betr, use_lch4
  use elm_varctl   , only: use_century_decomp, use_c13, use_c14, use_fates
  use elm_varctl   , only: iulog, create_crop_landunit, irrigate, create_wetland_landunit
  ...
  integer :: numwft      =  5      ! actual # of wetland functional type ((1) open water (2) mud (3) 3 different vegetation types)
  ...
  integer :: mxpft_in            ! maximum number of PFT's in input parameters. This variable is intended to replace mxpft_nc after including the wetland landunit
  integer :: numpft_veg_c = 10   ! maximum number crop pft within vegetated landunit
  ...
  integer :: wft_lb             ! In PFT arrays, lower bound of Patches on the wetland landunit
  integer :: wft_ub             ! In PFT arrays, upper bound of Patches on the wetland landunit
  integer :: wft_size           ! Number of Patches on wetland landunit
  ...
  subroutine elm_varpar_init()
    ...
    if (use_crop) then
       if (use_lch4) then   
          ...
          numpft      = mxpft - numwft   ! actual # of patches (without bare)
          numcft      =  36              ! actual # of crops
          numwft      = 0                ! actual # of wetland patches
          crop_prog   = .true.           ! If prognostic crops is turned on
       end if
    else
       if (use_lch4) then
          numpft      = numveg + numwft  ! actual # of patches (without bare)
          numcft      =   2              ! actual # of crops
          mxpft_in    =  24  + numwft    ! maximum number of PFT's when use_crop=False;
          crop_prog   = .false.          ! If prognostic crops is turned on
       else
          numpft      = numveg           ! actual # of patches (without bare)
          numcft      =   2              ! actual # of crops
          numwft      = 0                ! actual # of wetland patches
          mxpft_in    =  24              ! maximum number of PFT's when use_crop=False;
          crop_prog   = .false.          ! If prognostic crops is turned on
       end if
    end if
    ...
    if (create_crop_landunit) then
       if (create_wetland_landunit) then
    ...
    wft_lb = cft_ub + 1
    wft_ub = wft_lb + wft_size - 1

    max_patch_per_col= max(numpft+1, numcft, maxpatch_urb, numwft)
