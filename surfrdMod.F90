...
!-----------------------------------------------------------------------
  subroutine surfrd_special(begg, endg, ncid, ns,ntpu)
    ...
    use landunit_varcon , only : isturb_MIN, isturb_MAX, istdlak, istice, istice_mec
...
!-----------------------------------------------------------------------
    ...
    if (create_glacier_mec_landunit) then          ! call ncd_io_gs_int2d
       ...
       pctspec = pctlak + pcturb_tot + pctglc_mec_tot

    else

       pctglc_mec_tot(:,:) = 0._r8
       pctspec = pctlak + pcturb_tot + pctgla
    ...
    deallocate(pctgla,pctlak,pcturb,pcturb_tot,urban_region_id,pctglc_mec_tot,pctspec)
...
!-----------------------------------------------------------------------
    ...
    wt_lunit(begg:endg,1:max_topounits,istcrop) = arrayl(begg:endg,1:max_topounits)
     ...
     call ncd_io(ncid=ncid, varname='PCT_WETLAND', flag='read', data=arrayl, &
         dim1name=grlnd, readvar=readvar)
    if (.not. readvar) call endrun( msg=' ERROR: PCT_WETLAND NOT on surfdata file'//errMsg(__FILE__, __LINE__))
    wt_lunit(begg:endg,1:max_topounits,istwet) = arrayl(begg:endg,1:max_topounits) 
    ...
    wt_lunit(begg:endg,:,istwet) = wt_lunit(begg:endg,:,istwet) / 100._r8
