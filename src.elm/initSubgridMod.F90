...
contains
  ...
  subroutine add_landunit(li, ti, ltype, wttopounit)
    ...
    !
    ! !USES:
    use landunit_varcon , only : istsoil, istcrop, istice_mec, istdlak, isturb_MIN, isturb_MAX, istwet
    !
    ! !ARGUMENTS:
    ...
    if (ltype == istsoil .or. ltype == istcrop .or. ltype == istwet) then
  ...
  subroutine add_patch(pi, ci, ptype, wtcol)
    ...
    if (lun_pp%itype(li) == istsoil .or. lun_pp%itype(li) == istcrop .or. lun_pp%itype(li) == istwet) then
