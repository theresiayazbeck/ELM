...
contains
  ...
  subroutine elm_drv(doalb, nextsw_cday, declinp1, declin, rstwr, nlend, rdate)
    ...
    do nc = 1,nclumps
       ...
       !Add Wetland landunit
       call alt_calc(filter(nc)%num_wetsoilc, filter(nc)%wetsoilc, &
            temperature_vars, canopystate_vars)
       ...
       if(use_fates .or. use_cn) then
          ...
          !Add Wetland landunit
          call decomp_vertprofiles(bounds_clump, &
               filter_inactive_and_active(nc)%num_wetsoilc, &
               filter_inactive_and_active(nc)%wetsoilc, &
               filter_inactive_and_active(nc)%num_wetsoilp, &
               filter_inactive_and_active(nc)%wetsoilp, &
               soilstate_vars, canopystate_vars, cnstate_vars)
       end if
    ...
    do nc = 1,nclumps
       ...
       if(use_cn) then
          ...
          !Add Wetland landunit
          call veg_cs%Summary(bounds_clump, &
               filter(nc)%num_wetsoilc, filter(nc)%wetsoilc, &
               filter(nc)%num_wetsoilp, filter(nc)%wetsoilp, col_cs)
          call veg_ns%Summary(bounds_clump, &
               filter(nc)%num_wetsoilc, filter(nc)%wetsoilc, &
               filter(nc)%num_wetsoilp, filter(nc)%wetsoilp, col_ns)
          call veg_ps%Summary(bounds_clump, &
               filter(nc)%num_wetsoilc, filter(nc)%wetsoilc, &
               filter(nc)%num_wetsoilp, filter(nc)%wetsoilp, col_ps)
       ...
       elseif(use_fates)then
          ...
          !Add Wetland landunit
          call col_cs%ZeroForFates(bounds_clump,filter(nc)%num_wetsoilc, filter(nc)%wetsoilc)
          call col_ns%ZeroForFates(bounds_clump,filter(nc)%num_wetsoilc, filter(nc)%wetsoilc)
          call col_ps%ZeroForFates(bounds_clump,filter(nc)%num_wetsoilc, filter(nc)%wetsoilc)
       end if
    ...
    do nc = 1,nclumps
       ...
       call elm_drv_init(bounds_clump, &
            ...
            filter(nc)%num_wetsoilp, filter(nc)%wetsoilp, &
       ...
       if (use_erosion) then
          ...
          !Add Wetland landunit
          !call SoilErosion(bounds_clump, filter(nc)%num_wetsoilc, filter(nc)%wetsoilc, &
               !atm2lnd_vars, canopystate_vars, soilstate_vars,  sedflux_vars)
       ...
       if( .not. is_active_betr_bgc) then
          ...
          if (use_cn .or. use_fates) then
             ...
             call EcosystemDynNoLeaching1(bounds_clump,         &
                       ...
                       ch4_vars, photosyns_vars )
             !Add Wetland landunit
             write(iulog,*)'filter(nc)%num_soilc, filter(nc)%soilc,filter(nc)%num_soilp, filter(nc)%soilp',&
                  filter(nc)%num_soilc, filter(nc)%soilc,filter(nc)%num_soilp, filter(nc)%soilp
             write(iulog,*)'filter(nc)%num_wetsoilc, filter(nc)%wetsoilc,filter(nc)%num_wetsoilp, filter(nc)%wetsoilp',&
                  filter(nc)%num_wetsoilc, filter(nc)%wetsoilc,filter(nc)%num_wetsoilp, filter(nc)%wetsoilp
             write(iulog,*)'veg_pp%itype(filter(nc)%soilp(filter(nc)%num_soilp))', &
                  veg_pp%itype(filter(nc)%soilp(filter(nc)%num_soilp))
             write(iulog,*)'veg_pp%itype(filter(nc)%wetsoilp(filter(nc)%num_wetsoilp))', &
                  veg_pp%itype(filter(nc)%wetsoilp(filter(nc)%num_wetsoilp))
             !call EcosystemDynNoLeaching1(bounds_clump,         &
                       !filter(nc)%num_wetsoilc, filter(nc)%wetsoilc,  &
                       !filter(nc)%num_wetsoilp, filter(nc)%wetsoilp,  &
                       !cnstate_vars,            &
             ...
             if (use_elm_interface) then
                 ...
                 if (use_pflotran .and. pf_cmode) then
                    ...
                    !Add Wetland landunit
                    !call update_bgc_data_pf2elm(elm_interface_data%bgc,         &
                           !bounds_clump,filter(nc)%num_wetsoilc, filter(nc)%wetsoilc, &
                           !filter(nc)%num_wetsoilp, filter(nc)%wetsoilp,              &
                           !cnstate_vars, carbonflux_vars, carbonstate_vars,     &
                           !nitrogenflux_vars, nitrogenstate_vars,               &
                           !phosphorusflux_vars, phosphorusstate_vars,           &
       ...
       if (use_betr)then
          ...
          !Add Wetland landunit
          !call ep_betr%CalcSmpL(bounds_clump, 1, nlevsoi, filter(nc)%num_wetsoilc, filter(nc)%wetsoilc, &
               !col_es%t_soisno(bounds_clump%begc:bounds_clump%endc,1:nlevsoi), &
               !soilstate_vars, waterstate_vars, soil_water_retention_curve)
          ...
          if(is_active_betr_bgc)then
             ...
             !Add Wetland landunit
             !call ep_betr%PlantSoilBGCSend(bounds_clump, col_pp, veg_pp, &
                  !filter(nc)%num_wetsoilc,  filter(nc)%wetsoilc, cnstate_vars, &
                  !carbonflux_vars, c13_carbonflux_vars, c14_carbonflux_vars, nitrogenflux_vars, phosphorusflux_vars,&
                  !PlantMicKinetics_vars)
          endif
       ...
       if (use_betr) then
          ...
          if(is_active_betr_bgc)then
            ...
            !Add Wetland landunit
            !call ep_betr%PlantSoilBGCRecv(bounds_clump, col_pp, veg_pp, filter(nc)%num_wetsoilc, filter(nc)%wetsoilc,&
               !carbonstate_vars, carbonflux_vars, c13_carbonstate_vars, c13_carbonflux_vars, &
               !c14_carbonstate_vars, c14_carbonflux_vars, &
               !nitrogenstate_vars, nitrogenflux_vars, phosphorusstate_vars, phosphorusflux_vars)
       ...
       if (use_cn .and. doalb) then
           ...
           !Add Wetland landunit
           !call VegStructUpdate(filter(nc)%num_wetsoilp, filter(nc)%wetsoilp,   &
                !frictionvel_vars, cnstate_vars, &
                !canopystate_vars, crop_vars)
       end if
  ...
  subroutine elm_drv_init(bounds, &
    ...
    integer               , intent(in)    :: num_wetsoilp       ! number of wetland soil points in patch filter
    integer               , intent(in)    :: filter_wetsoilp(:) ! patch filter for wetland soil points
